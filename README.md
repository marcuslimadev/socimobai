...
LOCAL_MODEL_BASE_URL=http://127.0.0.1:8111
LOCAL_MODEL_API_KEY=
LOCAL_MODEL_NAME=socimobai-finetuned
...
`AI_PROVIDER=auto` tenta usar o modelo fine-tuned local primeiro, quando
`LOCAL_MODEL_BASE_URL` estiver configurado. Se ele falhar, tenta Hugging Face,
Ollama, Pollinations e, por ultimo, responde com o fallback local baseado nas
regras treinadas.

Opcoes:

- `huggingface` ou `hf`: usa o Hugging Face Router.
- `local_model` ou `finetuned`: usa a API Python do modelo fine-tuned.
- `auto`: modelo fine-tuned local -> Hugging Face -> Ollama -> Pollinations -> regras locais treinadas.
...
python scripts\generate_teacher_dataset.py --output data\teacher_socimob_qwen7b_v2.jsonl --target-examples 240
```
